...
- [ ] [#44](https://github.com/s2ngK/claude-asset-man/issues/44) 대출·예적금 탭 (`feat/44-accounts`) — 요구사항을 이슈 본문에 다시 썼고 v1 을 구현했다. **화면을 보고 추가 확인 중**
...
- [x] [#44](https://github.com/s2ngK/claude-asset-man/issues/44) 대출·예금·적금 계좌 v1 — **잔액 컬럼을 두지 않고** 연결된 거래에서 계산한다. 소유는 개인·열람은 그룹. 이자는 계산해 보여주되 **확정은 사람이** 한다(그 결정 하나로 단리/복리·세율 같은 필드가 스키마에서 전부 빠졌다)
...
- [x] #11 — 토큰이 만료되면 로그인 화면으로 돌려보낸다 (PR #35)

...
def create_user(payload: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    user = models.User(id=str(uuid.uuid4()), group_id=payload.group_id, display_name=payload.display_name, invite_code=code)
    db.add(user); db.commit(); db.refresh(user)
    return {"id": user.id, "display_name": user.display_name, "invite_code": code}


@router.get("/users")
def list_users(admin_key: str = Query(...), group_id: Optional[str] = Query(None), db: Session = Depends(get_db)):
    _check(admin_key)
    q = db.query(models.User)
    if group_id:
        q = q.filter(models.User.group_id == group_id)
    return [{"id": u.id, "group_id": u.group_id, "display_name": u.display_name, "invite_code": u.invite_code} for u in q.all()]

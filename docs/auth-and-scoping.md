...
이름만 작은 글씨로 적어두면 목록을 훑을 때 안 읽힌다. 그래서 셋을 겹쳐 쓴다.

| 표시 | 왜 |
|---|---|
| `나` 뱃지 | 글자다. 색을 못 보는 경우에도 읽힌다 |
| 옅은 배경색 | 훑을 때 덩어리로 잡힌다 |
| 왼쪽 강조 막대 | 배경색보다 눈에 먼저 들어온다 |

색은 **보조 수단**이다 — 뱃지 글자가 항상 함께 나온다.

> [!WARNING] 배경색은 불투명해야 한다
> 스와이프로 행이 밀리면 뒤에 빨간 삭제 배경이 깔린다. 반투명한 색을 쓰면 그게 비쳐 보인다.

> [!WARNING] 표시는 왼쪽 정렬 영역에만 넣는다
> 강조 막대는 `absolute` 라 어느 칸의 폭도 밀지 않고, 뱃지는 날짜 옆(왼쪽 정렬)에 있다.
> 오른쪽 금액 칸을 건드리면 **줄마다 숫자 끝이 어긋난다** → [함정과 교훈](pitfalls.md)
...
`accounts` 는 **개인 소유**(`user_id`)지만 목록은 그룹 전체가 본다 — 거래와 똑같다
(→ [대출·예금·적금 계좌](accounts.md)).
...
마지막 줄만 일부러 느슨하다. **배우자의 대출을 대신 갚는 것**은 공동 가계부에서 자연스럽다.
계좌의 조건(금리·만기·상환방식)을 바꾸는 것만 주인 몫이다.

> [!NOTE] 이 저장소에 "같은 그룹인데 안 보이는 데이터" 는 아직 없다
> 계좌를 개인 것으로 두면서도 열람을 그룹으로 연 이유가 이것이다. 숨기는 축을 하나 더
> 만들면 "모든 응답은 호출자의 그룹으로 한정된다" 는 불변식이 **그룹 + 소유자** 두 겹이
> 되고, 통계·달력이 그 계좌에 걸린 거래를 어떻게 다룰지까지 전부 따라 정해야 한다.

...
| 공통 | 드래그 끝에 오는 `click` 을 한 번 삼킨다 (`swallowClick`). 안 그러면 놓자마자 편집이 열린다. 새 `pointerdown` 마다 이 플래그를 턴다 — `click` 없이 끝난 제스처가 남긴 플래그가 **다음 클릭을 먹는** 것을 봤다 |
| 마우스 | `setPointerCapture` — 커서가 행 밖으로 나가도 `move`/`up` 이 계속 들어온다. **실패해도 삼킨다** — 캡처는 편의일 뿐인데 예외가 터지면 같은 제스처의 나머지 이벤트까지 망가진다 |
...
스와이프는 마우스로도 되지만 **끌 수 있다는 표시가 없다.** 그래서 `lg` 이상에서는 행에
마우스를 올리면 오른쪽 끝에 삭제 아이콘이 드러난다 (→ [반응형 레이아웃](responsive-layout.md)).

- 자기 내역에만 붙는다. 남의 내역에는 아예 렌더하지 않는다
- 클릭이 행의 편집 열기로 번지지 않게 `stopPropagation` 한다. 포인터 이벤트도 막아
  버튼을 누르는 것이 드래그로 해석되지 않게 한다
- 좁은 화면에는 내보내지 않는다 — 터치에는 hover 가 없어 항상 떠 있게 된다

> [!NOTE]
> 행을 그냥 클릭하면 아무 일도 안 일어난다. `MainView` 가 `onEdit` 을 넘기지 않아
> **편집 기능 자체가 아직 연결되지 않았다** (`TransactionItem` 쪽 구멍만 뚫려 있다).

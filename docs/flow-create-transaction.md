...
바깥 클릭은 **누른 곳이 배경 자신일 때만** 닫는다 (`e.target === e.currentTarget`).
안에서 눌렀다가 밖에서 손을 뗀 경우까지 닫으면 금액을 드래그로 선택하다 창이 사라진다.
...
계좌가 하나라도 있으면 모달 아래에 **계좌 연결** 칸이 뜬다. 고르면 이 거래가 그 계좌의
잔액을 움직인다 (→ [대출·예금·적금 계좌](accounts.md)).

- 기본값은 **연결 안 함**이다. 대부분의 거래는 계좌와 무관하다
- 목록에는 **진행 중인 계좌만** 나온다. 끝난 계좌에 새 상환을 붙일 일이 없다
- 동료의 계좌도 고를 수 있다 — 배우자의 대출을 대신 갚는 것은 공동 가계부에서 자연스럽다
- **지출일 때만 보인다.** 계좌를 움직이는 거래는 상환·납입·예치뿐이고 전부 지출이다.
  계좌를 고른 뒤 수입으로 바꾸면 연결이 저절로 풀린다 → [계좌](accounts.md)

**계좌에 카테고리를 정해뒀으면 그것도 함께 선택된다.** 상환·납입은 늘 같은 분류인데 매번
다시 고르게 할 이유가 없다. 정해두지 않은 계좌는 카테고리를 건드리지 않고, 자동으로 골라준
뒤에도 사용자가 바꿀 수 있다 — 강제가 아니라 기본값이다.

**대출을 고르면 `그중 이자` 칸이 함께 열리고, 이번 회차 예상 이자가 미리 채워진다.**
그 값은 `src/lib/accounts.ts` 가 금리·상환방식으로 낸 것이고 **저장되지 않는다** —
사람이 실제 낸 금액으로 고친다.

> [!NOTE] 금액을 찍기 전에는 경고하지 않는다
> 이자만 채워진 상태에서 금액이 0 이면 "이자가 금액보다 클 수 없습니다" 가 참이지만,
> 계좌를 고르자마자 빨간 글씨가 뜨면 **사용자가 뭔가 잘못한 것처럼 읽힌다.**
> 그 상태에서는 어차피 [완료] 가 잠겨 있다.

고른 계좌는 state 를 고쳐 맞추지 않고 **읽을 때 파생시킨다** — 목록에서 사라진 id 가 남아
있어도 화면과 state 가 어긋나지 않는다 (카테고리 선택과 같은 방식).
